...
- 카카오 카테고리 검색(카페 CE7, 음식점 FD6, 문화시설 CT1·관광명소 AT4)을 행정동 경계 사각형으로 호출하고, 좌표가 실제로 그 동네 경계 안에 있는 결과만 남깁니다.
- 제외 규칙: 카페는 카카오 분류가 `음식점 > 카페` 계열인 곳만 인정합니다. 키즈카페·만화카페·보드카페는 `가정,생활` 아래라 빠지고, 이름에 키즈카페·만화카페·보드게임·룸카페·스터디카페·PC방·무인카페·애견카페가 들어가도 제외합니다. 식당은 술집·뷔페·구내식당·패스트푸드·제과점을 제외합니다. 규칙은 `lib/server/kakao-places.ts`의 `classifyKakao`에 있습니다.
- 규칙을 바꾼 뒤 이미 들어간 장소를 걸러내려면 `npm run prune:kakao` (미리 보기는 `-- --dry-run`)를 실행합니다. 지운 뒤 다시 부족해진 동네와 그 동네만 다시 채우는 명령을 출력합니다.
