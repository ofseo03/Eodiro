...
export function parseTransit(xml: string, from: Place, to: Place, at: string, constraints: Constraints, requested: 'bus' | 'subway' | 'mixed'): Leg {
  ...
  for (const item of items) {
    ...
    // Official guide does not define access/transfer/wait inclusion. Never invent or double-count a total.
    options.push({ ...emptyLeg(from, to, at), status: 'ok', mode: requested === 'mixed' ? 'transit' : requested,
      distanceMeters: item.distance, providerMinutes: item.time ?? null, minutes: null, accuracy: 'provider', routes,
      timingNote: '공급자 소요시간의 접근·환승·대기 포함 여부 미검증. 전체 이동시간과 접근 도보 미확인', reason: null });
  }
  return options.sort((a, b) => (a.providerMinutes ?? Infinity) - (b.providerMinutes ?? Infinity))[0] ?? emptyLeg(from, to, at);
}
...
export async function getRoute(from: Place, to: Place, at: string, constraints: Constraints): Promise<Leg> {
  ...
  try {
    const leg = parseTransit(await fetchText(url), from, to, at, constraints, kind);
    return { ...leg, timingNote: [leg.timingNote, '환승 API는 요청 방문시각을 지원하지 않음; 현재 경로 조회 기준'].filter(Boolean).join('. ') };
